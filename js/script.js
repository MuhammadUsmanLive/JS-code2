...
document.getElementById("concatenation").onclick = function () {
    ...
    document.getElementById("input-box").innerHTML = '<p>"Sweet Dreams" + " " + "Usman" + "!"</p>'
...
document.getElementById("ifStatementNested").onclick = function(){
    ...
    if(userName === "Muhammad Usman"){
        ...
        if(password == 1130){
            ...
            document.getElementById("output-box").innerHTML = `<span class='text-danger'>${password}</span> is incorrect password. <br> correct password is <span class='text-success'>1130</span>`
        }
    }
    else{
        document.getElementById("output-box").innerHTML = `<span class='text-danger'>${userName} </span> is incorrect user name. <br> correct user name is <span class='text-success'> "Muhammad Usman"</span>`
    }
}
...
document.getElementById("login").onclick = function (){
    ...
    let passWord = +prompt("please enter password")
    if (userName === "Usman" && passWord === 1130 ){
        document.getElementById("form").innerHTML = `<span style='color:green; font-weight:bold;'>${userName}</span>`
        document.getElementById("output-box").innerHTML = `<span style='color:green; text-transform:capitalize;'>you are successfully logged in</span>`
    }
    else {
        document.getElementById("output-box").innerHTML = "Try using Usman as username and 1130 as a password"
    }
